...
import DashboardContent from "@components/Layouts/Content"
...
const LayoutContent = (props: LayoutProps) => {
    ...
    const clonedChildren = React.Children.map(props.children, (child) => {
        if (React.isValidElement(child)) {
            return React.cloneElement<any>(child, { adminLevel: props.adminLevel })
        }
        return child
    })
    ...
    return (
        <>
            ...
            <div style={{ width: "100%", padding: "15px 15px 15px 15px" }}>{clonedChildren}</div>
        </>
    )
}
